...
import { collection, doc, getDoc, updateDoc, setDoc, deleteDoc, onSnapshot, orderBy, addDoc, query, serverTimestamp } from '@firebase/firestore';
...
function Post({ postId, user, username, caption, imageUrl, noLikes, postUserId }) {
    ...
    useEffect(() => {
        if(postUserId) {
            ...
            const q = query(postRef, doc(postUserId))
    ...
    useEffect(() => {
        let unsubscribe;
        if (postId) {
            const postRef2 = collection(db, 'posts')
            const comRef = collection(db, 'comments')
            const q = query(postRef2, comRef, doc(postUserId), orderBy('timestamp', 'desc'))
    ...
    useEffect(() => {
        const postsUse = collection(db, 'posts')
        const likeUse = collection(db, 'likes')

        const q2 = query(postsUse, doc(postId), likeUse, doc(user.uid))
            getDoc(q2)
            .then(doc2 => {
                if (doc2.data()) {
                    if (show === 'like2') {
    ...
    const likeHandle = (event) => {
        ...
        const postsUse = collection(db, 'posts')
        const q3 = query(postsUse, doc(postId))
    ...
    useEffect(() => {
        if(postUserId) {
            ...
            const q = query(userRef, doc(postUserId))
