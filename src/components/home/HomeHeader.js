...
function HomeHeader({ user, selected }) {
  ...
  useEffect(() => { 
    ...
    const fetchUser = onSnapshot(userRef, snapshot => {
    ...
    const filteredUsers = (users) => {
      return users.filter((e) => {
        ...
        return e.user?.displayName.toLowerCase().indexOf(searchTerm.toLowerCase())
      })
    }
    //
    //if (users !== undefined) {
  ...
  useEffect(() => {
    ...
    const fetchPost = onSnapshot(postRef, snapshot => {
      snapshot.docs.forEach((doc) => {
        
        if (user && doc.data().username === user.displayName) {
          collection(db, 'posts').doc(doc.id).collection('comments').onSnapshot((snapshot2) => {
  ...
  return (
    ...
    <div className="homeHeader">
      <div className="homeHeaderLogoAndSearch">
        <Link to="/">
          <img src="https://i.ibb.co/72dN4JJ/Facebook-icon-2019-1.png" className="homeHeader__logo" alt='header logo' />
        </Link>
...

          {
            notifications.length === 0 ? (
              <div className="noNotifDiv">
                <img src={user && user.photoURL} alt='photo url'/>
